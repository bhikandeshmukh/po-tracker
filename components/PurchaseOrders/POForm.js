...
// Simplified: Only quantity fields, no price/amount
...
export default function POForm({ formData, onChange, onSubmit, loading, error, submitText = 'Save' }) {
    ...
    const totalOrderQty = formData.items.reduce((sum, item) => sum + (parseFloat(item.poQty) || 0), 0);
    const totalShippedQty = formData.items.reduce((sum, item) => sum + (parseFloat(item.qtySent) || 0), 0);
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div key={index} className="p-4 bg-gray-50 rounded-lg">
                            <div className="grid grid-cols-12 gap-4 items-end">
                                <div className="col-span-6 md:col-span-3">
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Order Qty *</label>
                                    <input type="number" value={item.poQty} onChange={(e) => handleItemChange(index, 'poQty', e.target.value)} required min="1"
                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div className="col-span-6 md:col-span-3">
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Shipped Qty</label>
                                ...
                                <div className="col-span-6 md:col-span-2">
                                    ...
                                    <input type="number" value={item.deliveredQty || 0} onChange={(e) => handleItemChange(index, 'deliveredQty', e.target.value)} min="0"
                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                                </div>
                ...
                {formData.items.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-gray-200 flex justify-end space-x-8">
                        <div className="text-right">
                            <p className="text-sm text-gray-500">Total Order Qty</p>
                            <p className="text-xl font-bold text-gray-900">{totalOrderQty}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-gray-500">Total Shipped Qty</p>
                            <p className="text-xl font-bold text-green-600">{totalShippedQty}</p>
                        </div>
                    </div>
                )}
            </div>
