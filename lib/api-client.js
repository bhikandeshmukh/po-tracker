...
class APIClient {
  ...
  async request(endpoint, options = {}) {
    ...
    const fetchRequest = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401 && !this._isLoggingOut) {
            this._isLoggingOut = true;
            if (typeof window !== 'undefined') {
              try {
                // Log system logout before clearing tokens and redirecting
                await this.post('/auth/logout', { reason: 'SYSTEM' });
              } catch (e) {
                console.error('Failed to log system logout:', e);
              }
              this.setToken(null);
    ...
    // Use deduplicator for GET requests to prevent parallel duplicates
    if (options.method === 'GET' || !options.method) {
      // Construct a unique key for the request including auth token to prevent cross-user leakage in weird edge cases
      // (though this is client-side, safer to be specific)
      const dedupKey = `${options.method || 'GET'}:${endpoint}:${this.token || 'anon'}`;
      return deduplicator.execute(dedupKey, fetchRequest);
    }
  ...
  async get(endpoint, params = {}) {
    ...
    const shouldBypassCache = params._t || params.refresh === 'true';
    ...
    // Cache the successful response if not explicitly skipped
    if (!shouldBypassCache && data.success) { // consistent with api response format
      cache.set(url, data);
    }
  ...
  post(endpoint, data) {
    ...
    const pattern = endpoint.split('/')[1]; // e.g., '/shipments' -> 'shipments'
    if (pattern) {
      cache.invalidatePattern(pattern);
      cache.invalidatePattern('dashboard'); // Also invalidate dashboard metrics
      cache.invalidatePattern('activities'); // Invalidate activities
    }
  ...
  put(endpoint, data) {
    ...
    if (pattern) {
      ...
      cache.invalidatePattern('activities');
    }
  ...
  async getRecentActivities(params = {}) {
    // Add timestamp to prevent caching only if explicitly requested or let default behavior handle it
    // Removing forced _t to allow cache to work by default unless 'refresh' is passed
    return this.get('/dashboard/recent-activities', params);
  }
