...
const purchaseOrderUpdateSchema = Joi.object({
    ...
    status: Joi.string()
        .valid('draft', 'submitted', 'approved', 'partial_sent', 'fully_shipped', 'partial_completed', 'completed', 'cancelled')
        .optional()
}).min(1); // At least one field must be provided

// Status Transition Rules
// Note: 'partial_sent' is the standard status for partially shipped POs
// 'partially_shipped' kept for backward compatibility but maps to same behavior
const STATUS_TRANSITIONS = {
    draft: ['submitted', 'approved', 'cancelled'],
    submitted: ['approved', 'cancelled'],
    pending: ['approved', 'cancelled'],
    approved: ['partial_sent', 'fully_shipped', 'completed', 'cancelled'],
    partial_sent: ['fully_shipped', 'completed', 'partial_completed', 'cancelled'],
    fully_shipped: ['completed'],
    completed: [],
    partial_completed: [],
    expired: [],
    cancelled: []
