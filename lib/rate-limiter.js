...
// Auth rate limiter for login/register (more lenient for testing)
const authLimiter = createRateLimiter({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 500, // 50 requests per 15 minutes
...
const strictRateLimiter = createRateLimiter({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 500, // 10 requests per 15 minutes
...
const lenientRateLimiter = createRateLimiter({
    windowMs: 60 * 1000, // 1 minute
    max: 500, // 300 requests per minute
    message: 'Too many requests, please slow down'
});
...
const bulkOperationLimiter = createRateLimiter({
    ...
    max: 500, // 5 bulk operations per 5 minutes
    message: 'Too many bulk operations, please wait before trying again'
});

// Helper function to apply rate limiter as async
