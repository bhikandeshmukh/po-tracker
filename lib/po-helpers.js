...
async function recalculatePOTotals(poId, transaction = null) {
    ...
    itemsSnapshot.docs.forEach(doc => {
        const item = doc.data();
        const lineTotal = (item.poQuantity || 0) * (item.unitPrice || 0);
...
async function createPOWithTransaction(poData, items, user) {
    ...
    // Update dashboard metrics asynchronously (don't wait for it)
    updateDashboardMetrics().catch(err => console.error('Failed to update dashboard metrics:', err));
...
async function updateDashboardMetrics() {
    try {
        const [posSnapshot, activePosSnapshot, shipmentsSnapshot, inTransitSnapshot, deliveredSnapshot, returnsSnapshot, vendorsSnapshot, activeVendorsSnapshot, transportersSnapshot] = await Promise.all([
        ...
        const metrics = {
            ...
            pendingApprovalPOs: posSnapshot.docs.filter(doc => doc.data().status === 'pending_approval').length,
            totalPOAmount,
            ...
            pendingReturns: returnsSnapshot.docs.filter(doc => doc.data().status === 'pending').length,
            completedReturns: returnsSnapshot.docs.filter(doc => doc.data().status === 'completed').length,
            totalReturnAmount,
